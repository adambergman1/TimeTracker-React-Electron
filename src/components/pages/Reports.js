...
import DatePicker from '../reports/DateRangePicker'
...
export class Reports extends Component {
  ...
  calculateCost = (rate, diff) => {
    ...
    const sum =
      elapsedTime.length > 1 ? elapsedTime.reduce((a, b) => a + b) : elapsedTime
        ...
        return (
          <div className='collection-item row' key={project.id}>
            ...
            <div className='total-time col s2'>
              <span>
                {project.tasks.length
                  ? diff.length
                    ? getElapsedTime(diff.reduce((a, b) => a + b))
                    : '0:00:00'
                  : '0:00:00'}
              </span>
            </div>
            <div className='col s3 right-align'>
              {project.rate.length ? (
                <span>{this.calculateCost(project.rate, diff)}</span>
              ) : (
                ''
              )}
            </div>
        ...
        <div className='section red darken-3'>
          <div className='container'>
            <div className='row mb-1'>
              <div className='absolute'>{this.props.children}</div>
        ...
        <div className='section'>
          <div className='container'>
            <DatePicker
              ...
              onStartChange={this.changeStartDate}
              onEndChange={this.changeEndDate}
            />
            <div className='collection-heading row'>
              <div className='name col s5'>
                <span>Project</span>
              </div>
              <div className='hourly-rate col s2'>
                <span>Rate/h</span>
              ...
              <div className='earned-money col s3 right-align'>
                <span>{currency ? currency.code : 'Earned'}</span>
              </div>
            </div>
            <div className='collection'>{projectsToDisplay}</div>
          </div>
        </div>
      </main>
    )
